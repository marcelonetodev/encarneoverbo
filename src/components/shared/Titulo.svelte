<script lang="ts">
  ...
  export let principalMenor: any = null;
  export let secundario: any = null;
</script>

<div
  class="flex flex-col p-5  text-black  dark:text-white"
>
  {#if key}
    <blockquote class="text-3xl sm:text-4xl md:text-5xl font-black">
      ...
      <p
        class="p-2 text-sm sm:text-base text-center font-light dark:text-zinc-400"
      >
        {after}
      </p>
    </blockquote>
  ...
  {#if principal}
    <h1 class="text-3xl dark:text-white/70 font-black">
      {principal}
    </h1>
  {/if}
  {#if principalMenor}
    <h1 class="text-base dark:text-white/70 font-black">
      {principalMenor}
    </h1>
  {/if}
  {#if secundario}
    <p class="p-2 text-sm sm:text-base dark:text-zinc-400/80">{secundario}</p>
  {/if}
</div>
